...
const (
	...
	// and the payload retained by the sniffer. Coverage stays in Aster's sorted
	// IntRanges representation. The 64 KiB payload budget matches TCP sniffing
	// and the largest pooled buffer, and aligns with the Initial CRYPTO limits used by
...
// QuicSniffer is kept as an alias for Aster callers using the historical name.
type QuicSniffer = QUICSniffer
...
// NewQuicSniffer preserves Aster's existing constructor while the canonical
// spelling follows the upstream QUIC initialism.
func NewQuicSniffer(snifferConfig SnifferConfig) (*QUICSniffer, error) {
