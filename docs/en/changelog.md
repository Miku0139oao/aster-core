...
- **Relay headroom no longer inflates the slab (review fix):** `copyExtendedAdaptive` sized its buffer as `payload + headroom`, and `buf.NewSize` rounds that up to the next allocator class. Destinations that declare headroom without an MTU (VLESS-vision, WebSocket, gRPC) therefore held an 8 KiB slab when idle and a 64 KiB slab at full rate, where sing used one 32 KiB slab with the headroom carved inside. Sizing now follows sing: the size class is the total, headroom is carved from it. Byte-stream sources start at a 4 KiB slab (8 KiB when headroom is large) and grow to `RelayBufferSize`; MTU destinations keep `MTU + headroom`, and shrinks land back on a power of two. Measured WebSocket-like slab sequence `4096→8192→16384→32768` (previously `8192→…→65536`); vision-like (headroom 2424) `8192→…→32768` with a 30344-byte full-rate payload. `TestIdlePipeRelayHeapPerConn` now waits for each copier to enter `Read` instead of `time.Sleep`; the figure is unchanged (about 12.6 KiB/conn).
