...
- **TCP process RAM was not shown to improve or return to idle within 60 seconds**. The 1,000-held-connection median was actually +1.8%. Releasing objects or returning buffers to pools, and lower B/op, do not mean immediate return of memory to the OS. After connections close, allocation drops to ~0 so Go does not GC on its own and RSS stays near the peak. If a one-shot GC pause is acceptable, set `experimental.idle-memory-scavenge` (off by default; see [Configuration](/en/reference/configuration)). That option is not part of the 2026-09-05 A/B and must not be written back into those tables.
