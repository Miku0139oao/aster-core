...
Governance state and reports live in bbolt. Ordinary traffic is checkpointed every five minutes. Overage, reset, configuration changes, and a clean shutdown commit immediately. Archived time buckets use partitioned Zstandard compression. Each block has an original length and CRC32C, and queries decompress only the needed segments.
...
The `kernel_direct` object from `GET /api/aster/capabilities` is version `4` (v4 was never published; this release adopted it directly and did not jump to 5). `deprecated_fields` includes `proxy_traffic`.
...
Report queries accept `key`, `granularity=hour|day|month`, and `from` / `to` Unix timestamps. Cross-dimension keys are joined with `|`, for example `device:phone|rule:video-rule`.
