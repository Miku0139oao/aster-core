...
- UDP NAT 使用 typed key、有上限的增量 reverse mapping，並合併 deadline refresh；不再為每個新目的地複製整張 map
...
- 合併後的 IP-CIDR／GeoIP range 使用二分搜尋；Kernel DIRECT 在下一筆 TTL 前不再掃描完整 cache
- QUIC sniffing 以固定上限 bitmap 追蹤 CRYPTO coverage，避免稀疏 fragment 造成 range amplification
