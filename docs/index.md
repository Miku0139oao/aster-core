...
features:
  ...
  - icon: ⚡
    title: 核心轉送更省、更快
    details: 實際 OpenWrt 主機對 Mihomo 1.19.30 同機測試：TCP 核心轉送快約 4%，AnyTLS 封包處理快約 1.3–2.0 倍。
...
| --- | ---: | --- |
| TCP 核心轉送 | **約快 4%** | 搬資料時少做一些無用工作，並消除每次轉送的記憶體配置 |
| AnyTLS 封包打包 | **快約 1.3–2.0 倍** | 高速傳輸或很多小封包時，打包成本更低 |
...
> [!IMPORTANT]
> 這不代表 Speedtest 會快 5.7 倍。5.7 倍是「準備一個 UDP 封包」這個核心小步驟；最接近整體資料轉送的 TCP 測試約快 4%。測試用的 Ryzen 7 5825U 軟路由仍比許多家用路由器強，低階裝置的絕對速度會更低，不能直接套用這些數字。另有以單核 25% CPU 時間、512 MiB 記憶體模擬弱硬體的測試，五項優化仍全部有效。整體趨勢是硬體越弱，省掉的 CPU 工作與記憶體配置越有感；首頁仍保留較保守的未限速結果。
